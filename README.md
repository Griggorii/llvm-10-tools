...
llvm-10-tools-full download: https://yadi.sk/d/Cgk8MDs9iyW84A llvm пропадёт wayland так как системы щас иногда привязаны к llvm больше по графическим стекам , этот llvm экспериментальный потому как надо найти что бы завести потому что его не может программа задетектить , скорее на другой системе сделать маке под llvm-10 в эту перебросить и раскомпилировать тогда можно получить модификацию. Этот llvm может перестройть систему как пример звуковой стек должен быть лучше после его применения и использовать надо на данный момент либо 20.10-Ubuntu-Groovy-Gorilla либо 20.04-Ubuntu-Focal-Fossa<-но , полностью обновленную fossa другие же более старые дистрибутивы могут использовать  llvm-9, llvm-8 и далее ниже что уже не подойдёт вот только libffi6 и libhandy ещё бы не трогали , а то прогресс виден https://www.youtube.com/watch?v=AGpgz0mU6PY .

В ходе разработки llvm для mesa я наткнулся что mesa нуждается в инклюде pthread_np.h

_______________________________________________________________________________________________________________________

Один пример из llvm-10 привязок | example dependency llvm-10 off wayland

dpkg-shlibdeps: предупреждение: у пакета debian/gnome-control-center/usr/libexec/gnome-control-center-search-provider можно удалить бесполезную зависимость от libwayland-server.so.0, если исключить её из компоновки (так как не используется ни одного из её библиотечных символов)
...

{
  ...
  __interceptor___strtouq_internal;
  __interceptor___strtouq_l;
  __interceptor___strxfrm_l;
  __interceptor___tls_get_addr;
  __interceptor___uflow;
  __interceptor___underflow;
  ...
  __interceptor_strtoumax;
  __interceptor_strtouq;
  __interceptor_strtouq_l;
  __interceptor_strxfrm;
  __interceptor_strxfrm_l;
